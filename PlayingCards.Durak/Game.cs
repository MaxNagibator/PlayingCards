...
public class Game
{
    ...
    /// Проигравший.
    /// </summary>
    public Player? LooserPlayer { get; private set; }

    /// <summary>
    /// Карты на столе.
    /// </summary>
    public List<TableCard> Cards { get; private set; }
    ...
    internal void Attack(Player player, List<Card> cards)
    {
        ...
        if (cards.Any(card => !IsRankOnTable(card)))
        {
    ...
    {
        var playersWithCards = Players.Where(x => x.Hand.Cards.Count > 0).ToArray();

        if (playersWithCards.Length != 1 || Deck.CardsCount != 0)
        {
            return;
        }
